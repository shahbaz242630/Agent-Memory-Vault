...
**Last updated:** 2026-05-08 (T0.2.0 Phase 0c COMPLETED — at-rest spike v2 ALL STAGES PASS, runtime-confirmed `ObjectStoreProvider` integration; Stage E diagnostic discovered ADR-039 production bug, fixed in same commit: Prune-alone insufficient for partial-fragment deletes — amended to Compact+Prune)
...
**Active task:** **T0.2.0 Phase 0d — production-wire `SealedFileStoreProvider`.** Phase 0c spike v2 ALL STAGES PASS (commit pending Shahbaz approval); 221/221 vault-storage tests pass (220 + new `delete_partial_fragment_physically_removes_content_per_adr_039` regression pin). Runtime-confirmed that `ObjectStoreProvider` registered for unknown scheme `vault-sealed://` actually intercepts BOTH write AND read flows in lance-io 4.0 (refutes v1 `LocalObjectReader` bypass FAIL). Stage E 2×2 diagnostic surfaced + we fixed an ADR-039 production privacy bug: the Phase 0b `OptimizeAction::Prune`-alone implementation reports `data_files_removed: 0` for partial-fragment deletes (Memory Vault's actual API pattern — single-id delete leaves siblings in the same fragment); `Compact + Prune` is the correct sequence (`data_files_removed: 1` empirically). Production code amended in this same commit + new partial-fragment regression test pins the corrected behavior.
...
| **0d** | PENDING | Wire `SealedFileStoreProvider` into production `LanceVectorStore::open()` alongside ADR-038 `upsert_lock`. Add production-integration regression tests (open-with-correct-key works, wrong-key fails closed, on-disk-sealed pin, ADR-039-through-sealing). Promote `lance-io` / `lance-core` / `object_store` / `dryoc` / `bytes` / `url` / `walkdir` from spike-local dev-deps to workspace `[workspace.dependencies]` per `feedback_spike_playbook_for_unknowns.md` (production code consumes them). |
| **0e** | PENDING | ADR-037 (lancedb upgrade rationale) + ADR-008 amendment (at-rest extension locking K3 KDF + Finding 2(c) AAD + ObjectStoreProvider integration) + T0.2.0 plan paragraph iteration 1. ADR-038 already drafted at 0a-fix, will cross-link from ADR-037. ADR-039 amendment text already drafted in this section below — Phase 0e will fold into final ADR-039 archive entry. |

---

## Session-end working-tree state (2026-05-08, single bundled commit pending Shahbaz approval)

**Phase 0c (at-rest spike v2) — files modified:**
- `crates/vault-storage/examples/at_rest_spike.rs` (NEW, ~1530 LOC): five-stage spike. Stages A/B/C runtime-confirm `ObjectStoreProvider` integration on `vault-sealed://`. Stage D exercises adversarial sweep + ADR-039-through-sealing with Compact+Prune. Stage E 2×2 diagnostic matrix that surfaced the ADR-039 partial-fragment bug. Pre-flight Cargo.lock snapshot + Finding-1 (`WrappingObjectStore` still exists in lance-io 4.0) explicit at top-of-file. Runtime PASS confirmed 2026-05-08 (release binary). Locked decisions from V1 spike (AAD, K3 KDF, sealing shape, granularity, dryoc path #1) lifted verbatim from `at_rest_spike.rs.v1_fail_disabled`.
- `crates/vault-storage/Cargo.toml`: re-added 7 spike dev-deps at lancedb 0.27.2 transitive versions (verified by `cargo tree -p lancedb` 2026-05-08): `lance-io = "=4.0.0"`, `lance-core = "=4.0.0"` (plan-amendment from iteration-2 "lance-io only" — required because trait method signatures use `lance_core::{Error, Result}` and lance-io doesn't re-export them publicly; approved by Shahbaz mid-flight 2026-05-08), `object_store = "=0.12.5"`, `dryoc = { workspace = true }`, `bytes = "=1.11.1"`, `url = "=2.5.8"`, `walkdir = "=2.5.0"`. All spike-local; Phase 0d promotes the production-consumed subset to `[workspace.dependencies]`.

**Phase 0c discovery → ADR-039 amendment (Compact+Prune) — files modified:**
- `crates/vault-storage/src/vector_store.rs`:
  - line 57: import `CompactionOptions` alongside `OptimizeAction`.
  - lines 350-395 (the `delete()` body): replaced `OptimizeAction::Prune`-alone with `OptimizeAction::Compact { options: CompactionOptions::default(), remap_options: None }` followed by `OptimizeAction::Prune { older_than: zero, delete_unverified: true, error_if_tagged_old_versions: false }`. Comment block expanded with the Phase 0c Stage E 2×2 diagnostic finding + reasoning + measured Compact+Prune OptimizeStats (`compaction.fragments_removed: 1, fragments_added: 1, files_removed: 2, files_added: 1` followed by `prune.bytes_removed: 159379, data_files_removed: 1`).
  - new test `delete_partial_fragment_physically_removes_content_per_adr_039` (companion to existing `delete_physically_removes_content_per_adr_039`): writes 10 rows to one boundary, deletes 5 by single-id call (partial-fragment delete — Memory Vault's actual API pattern), asserts no pre-delete data file's content-hash survives post-delete using BLAKE3 hash-set difference. Fails loudly under Prune-alone (regression pin for the Stage E discovery).
  - existing `delete_physically_removes_content_per_adr_039` test comment updated to reference the new Compact+Prune sequence in its failure message.

**Phase 0c — files NOT modified (intentional):**
- `Cargo.toml` (workspace): no changes. Phase 0d will promote spike-local deps to `[workspace.dependencies]` when production code consumes them.
- `crates/vault-storage/examples/at_rest_spike.rs.v1_fail_disabled`: KEEP AS-IS — executable failure documentation referenced by upcoming Phase 0e ADR-008 amendment.

**HANDOFF.md updates:**
- This file — Phase 0c status flip from PENDING → DONE, Phase 0d entry expanded, Active Task header rewritten, ADR-039 forward-pointer text amended (Compact+Prune sequence, Stage E discovery cross-reference), test count 220→221, ADR-039 amendment block added below ADR-038.
