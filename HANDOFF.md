...
## 1 · 🟢 NEXT SESSION OPENER — build A1 cold-archive (the anti-bloat tool), then commit ADR-083 + re-run 1k (optional)

> **🆕 2026-06-20 (session 8) — CONTRADICTION OVER-RETENTION GUARD (ADR-083, §8.15) BUILT + DoD GREEN + REAL-Phi-4 VERIFIED. ⚠️ UNCOMMITTED — founder pre-approved commit+push; stage when ready. The "1k cosine-prune" that opened this session was INVESTIGATED + KILLED by measurement (it was the wrong fix); the real work became a correctness fix.**
>
> ### ✅ DONE this session
...
> 1. **Commit + push** the 3 working-tree files (founder pre-approved): `crates/vault-consolidator/src/phases/contradiction.rs` (the fix + `real_phi4_distinct_events_not_retired` probe), `crates/vault-app/tests/scale_eval.rs` (the `probe_contradiction_pair_distribution` diagnostic — rides as ADR-083's evidence per [[feedback_commit_only_with_tested_fix]]), `HANDOFF.md`. Then `gh run list --workflow=ci.yml -L 1` to CI-verify. **(If this session already committed, skip.)**
> 2. **Build A1 cold-archive** — now the priority, as the structural anti-bloat tool the "keep when unsure" posture leans on (founder asked the bloat question 2026-06-20). Facts untouched ~`archive_after_days` (365) move OUT of default retrieval to an archive store. First-class `Memory` state change (schema + retrieval-filter) → **policy ADR before code**. See `phases/decay.rs` module doc + §5 item 2.
