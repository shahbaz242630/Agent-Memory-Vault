...
**Last updated:** 2026-06-04 (**`memory_search` RECALL-FIRST (ADR-067) + topic labels K-means→connected-components (ADR-068) + calibration fixture grown to 24 cases.** Rough edges #1/#3/#4 from the 2026-06-02 opener closed in one batch; all DoD gates green + the headline search fix verified live on real BGE (`search "food"`→Japanese cuisine, `search "amber"`→favorite color). `memory_search` no longer hard-gates on BM25 — it returns the semantic-backed hybrid candidates (the single-token/no-overlap `[]`-in-2ms bug is gone). Topic discovery groups by cosine connected-components (floor 0.70, provisional) instead of force-bucketing K-means, so unrelated facts stop getting mislabeled. **Remaining: (2) read/search latency [deferred, post-correctness]; calibrate the 0.70 topic floor + −2.5 no-signal floor on real fill data; consider removing now-unreferenced `AbstainingRetriever`.** AWAITING commit+push approval. Prior openers HISTORICAL.)

---

## 🟦 NEXT SESSION OPENER (2026-06-04) — **`memory_search` RECALL-FIRST (ADR-067) + topics K-means→connected-components (ADR-068) + fixture grown. DoD-GREEN + FULL §7 DOGFOOD GREEN, committed+pushed.** — READ THIS FIRST
...
Once approved + pushed, confirm the CI run goes `success` (`gh run list --workflow=ci.yml -L 1`) before staging anything new ([[broken-ci-is-regression-not-techdebt]]). The weekly `real-model smoke` cron is still red on the pre-existing Phi-4 GGUF download race (NOT this arc — fix is `--test-threads=1` on the 3 smoke tests; deferred).
...
### ✅ SHIPPED THIS SESSION (uncommitted; DoD-green)
...
1. **Calibrate the floors on real fill data** — the 0.70 topic floor (ADR-068) + the −2.5 no-signal floor (ADR-066). Run the `#[ignore]` `read_quality_eval` / `recall_first_go_no_go` harness against the grown 24-case fixture; capture labelled prod judgments as the vault fills.
2. **Latency (#2, deferred)** — read ~4-70s + search now runs the hybrid (semantic embed per query, ~70-160ms; fine). Reranker on CPU is the read cost. int8/GPU/DirectML per deployment mode.
3. **Remove `AbstainingRetriever`** (+ `AbstainConfig`) — production-unreferenced after ADR-067. Focused cleanup; check vault-retrieval re-exports + tests.
4. **Re-dogfood `memory_search`** in Claude Desktop on a clean vault to confirm the single-token cases (`amber`, `C7UNI`) surface live (the unit test proves it through the same stack; a live run closes the loop).

### Tech debt logged
- `AbstainingRetriever` / `AbstainConfig` production-unreferenced (ADR-067) — remove in a focused pass.
- 0.70 topic floor + −2.5 no-signal floor both provisional — calibrate on real fill.
