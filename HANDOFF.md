...
## 1 · 🟢 NEXT SESSION OPENER — verify CI green on the A1 push, then pick the next arc (recommend: live dogfood of A1 + ADR-083, then cross-device sync)

> **🆕 2026-06-21 (session 9) — A1 COLD ARCHIVE (ADR-084, §8.16) BUILT + FULL DoD GREEN on a fresh cold build. UNCOMMITTED at time of writing (commit approval pending). The anti-bloat tool the "keep when unsure" posture leans on.**
>
> ### ✅ DONE this session
...
> 1. **Commit + push** the A1 set (if not already done at session close — see "commit pending" below) then **verify CI green**: `gh run list --workflow=ci.yml -L 1` should be `success` on the A1 commit before any new work.
