...
**Last updated:** 2026-06-02 evening (**RECALL-FIRST READ SHIPPED (ADR-066) — synonym-gap over-abstention RESOLVED, full §7 dogfood GREEN on real Phi-4.** The read stopped trying to be a strict relevance *authority* via the 0.6B cross-encoder (which provably can't separate hard synonym leaps from ambiguous guards at any single floor) and went **recall-first**: the reranker re-orders + drops only deep no-signal junk (floor `RERANK_NO_SIGNAL_FLOOR = -2.5`, was a precision floor of 0), and the calling agent makes the fine relevance call. **The exact bug that nearly scrapped the project — "what food does the user like?" → "favourite cuisine is Japanese" — now SURFACES live in Claude Desktop**, while A6 no-signal still abstains, A5 contradiction + dedup still hold, and C4 is closed (100K stores intact). Make-or-break sprint; it passed. **Remaining rough edges → next session (see 🟦 opener): (1) `memory_search` returns empty across the board [TOP]; (2) read latency ~4-70s CPU reranker; (3) K-means topic mislabels (cosmetic); (4) re-validate the -2.5 floor + grow the labelled fixture as the vault fills.** Prior openers HISTORICAL.)

---

## 🟦 NEXT SESSION OPENER (2026-06-02 evening) — **RECALL-FIRST READ SHIPPED (ADR-066), §7 DOGFOOD FULLY GREEN. NEXT: fix `memory_search` (same recall-first fix) + 3 rough edges.** — READ THIS FIRST
