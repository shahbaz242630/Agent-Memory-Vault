...
> ### ▶️ START HERE — current state (2026-07-20, session 23 close)
>
> **In one line:** the app now fetches its own recall engine on first run, with progress, a gate, and a retry — so the engine story is complete end-to-end; **the only thing left between us and beta users is that there is still no installer.**
>
> **🔴 UNCOMMITTED AT SESSION CLOSE — see "banked below" for the file list.** All five DoD gates are GREEN (build 0 warnings 2m33s · `cargo test -p vault-core -p vault-embedding -p vault-llm -p vault-retrieval -p vault-mcp -p vault-tauri` **354 passed / 0 failed** · clippy `--all-targets -D warnings` 0 warnings 40s · `fmt --all --check` clean · no `Cargo.lock` drift). ⚠️ **`crates/vault-tauri/src/commands/engine.rs` is a NEW UNTRACKED file — use `git add -A`, not `git add -u`** (same trap as session 21's `frontend_contract.rs`).
>
> **🚦 CI:** session 22's `b95cc6e` push run `29759119324` verified **`success`** (59m43s) before any of this session's work was staged. The weekly `schedule` run remains independently red — see carried threads; match on `event=push` AND the SHA.
>
...
>
> ### 🔬 NOT YET LIVE-VERIFIED — DO THIS FIRST NEXT SESSION
>
> Everything above is gate-green but **has not been run against the real app**. Session 22's lesson was that both of its architectural bugs were found by RUNNING, not reading. Before packaging:
> 1. Move `<vault>/models/qwen3-reranker-0.6b-seq-cls/` aside, launch via `scripts/run-desktop-dev.ps1`, and confirm: progress row appears and advances, onboarding gate holds, search still WORKS (degraded) during the fetch, and the reranker engages after completion **without a relaunch** — that last one is ADR-089's central claim and is currently proven only by unit test.
> 2. Confirm a warm launch shows NO progress row (files present ⇒ honest UI shows nothing).
> 3. Kill the network mid-fetch and confirm onboarding completes degraded rather than stranding.
> 4. Re-run the live download test after any URL/hash change: `cargo test -p vault-tauri --test reranker_download -- --ignored --nocapture`.
>
