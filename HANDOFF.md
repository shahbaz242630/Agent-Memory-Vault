...
**Last updated:** 2026-05-17 (T0.2.3 close commit 9 ship, CI watch pending) — **T0.2.3 read-time-pipeline code SHIPPED across commits 3 + 4; commits 5+6+7+8 all failed Windows CI on llama-cpp-sys-2's `vulkan-shaders-gen` ExternalProject_Add try_compile hitting `error C1083` + VCEnd in CMakeTestCCompiler. Each fix attempt — humbletim → chocolatey → direct LunarG installer → `CMAKE_TRY_COMPILE_CONFIGURATION=Release` env — confirmed Linux + macOS work fine and falsified its own Windows hypothesis. **Commit 9 fix-forward SHIPPED — switch Windows runner image from `windows-latest` (Windows Server 2022 + VS 2022 / MSBuild 17, where the bug lives) to `windows-2025` (Windows Server 2025 + VS 2026 / MSBuild 18, the exact runner ggml-org/llama.cpp's own release.yml CI uses to ship Windows Vulkan binaries).** Single-token swap × 3 jobs. CI watch pending.** macOS green throughout (Metal). **T0.2.7 Phase 0 t028a security spike PASSED locally: 3078 of 3078 Lance HNSW index files sealed via `vault-sealed://` envelope.** **Next session opens with: (1) confirm commit 9 CI green across the 3-OS matrix → T0.2.3 CI-side closed, (2) T0.2.7 Phase 1 t028b benchmark spike unblocked. If commit 9 still red, candidates B (Ninja generator on windows-2025), C (downgrade llama-cpp-2 to v0.1.139), D (accept Windows-CI-Vulkan gap + document scope in ADR) are next-step options.** See "Next-session opener" below.
...
| 9 | `<pending push>` | `.github/workflows/ci.yml` fix-forward (Windows-only, Linux + macOS unchanged): switch the Windows runner image from `windows-latest` (= Windows Server 2022 + VS 2022 / MSBuild 17 until GitHub's 2026-06-08→06-15 migration) to `windows-2025` (= Windows Server 2025 + VS 2026 / MSBuild 18). Applied to all 3 Windows jobs (clippy + build-and-test matrices + real-model-smoke `runs-on`). **Mechanism:** the OpenCV-class VCEnd batch-label + `error C1083` bug surfaced at commits 6-8 is in VS 2022's MSBuild 17 vcxproj custom-build batch-wrapper handling. VS 2026's MSBuild 18 is a fundamentally different binary with different generated-batch-file behavior. ggml-org/llama.cpp's own `release.yml` CI ships Windows Vulkan binaries from `windows-2025` runners successfully — direct evidence the bug doesn't manifest on the newer image. Includes top-of-file comment block documenting the rationale + escape-hatch candidates B/C/D for future sessions. Bundled per admin-rides-with-code: commit 9 HANDOFF update only (no Rust code changes). | 🟡 watch pending |
...
## Next-session opener — T0.2.3 commit 9 CI verify + T0.2.7 Phase 1 t028b (drafted 2026-05-17 commit 9 ship)
