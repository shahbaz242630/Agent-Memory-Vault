...
## 1 · 🟢 NEXT SESSION OPENER — 📦 MSI PACKAGING (the last beta blocker). Sessions 2–18 detail is archived → `HANDOFF_V0.2_PART3_ARCHIVE.md`.

> ### ▶️ START HERE — current state (2026-07-22, session 24 close)
>
> **In one line:** the engine story is not just built but **RUN and proven** — first-run download, degrade-while-fetching, engage-without-relaunch, warm-up, and honest readiness are all live-verified; **the only thing left between us and beta users is that there is still no installer.**
>
> **✅ SESSION 24'S WORK IS COMMITTED AND PUSHED.** 12 files, +561/−15 (ADR-090). All five DoD gates were GREEN before staging: build 0 warnings **46m37s** · `cargo test -p vault-embedding -p vault-app -p vault-tauri` **133 passed / 0 failed** (+8 new, each verified to actually execute rather than pass vacuously) · clippy `--workspace --all-targets -D warnings` 0 warnings **25m03s** · `fmt --all --check` clean, re-run last with nothing between it and staging · no `Cargo.lock` drift · no untracked files.
>
> **🚦 CI PRECONDITION — CHECK THIS FIRST.** Run `gh run list --workflow=ci.yml -L 4`, find the `push` run for session 24's SHA, and confirm `success` **before staging anything new**. Not a formality: this commit touches `vault-embedding` (low in the stack → whole-workspace rebuild), adds a public enum to `vault-app`, and adds two Tauri commands whose four-file wiring only CI's clean-room matrix fully re-validates. **Do not confuse it with the weekly `schedule` run** — that job is independently red (tech-debt #6, `real-model smoke`, confirmed again 2026-07-20 and 2026-07-22) and says nothing about our commit; match on `event=push` AND the SHA.
>
> **Session 23's `9adf104` run `29766980235` was verified `success`** (1h3m23s) at session-24 open — that precondition is discharged, not outstanding.
>
> **📋 SESSION-24 HOUSEKEEPING (expected, not a problem):** the SHA + CI-run-id lines above are filled in *after* the commit exists, so a small `HANDOFF.md` edit may be sitting uncommitted at session open. That is the established pattern in this repo (session 23 opened the same way) — **it rides with the next code commit** per [[feedback_admin_changes_ride_with_code]]. Do not give it its own commit.
>
> ### 🎯 DO THIS FIRST NEXT SESSION
>
> 1. **Confirm CI green** (above). Blocks everything else.
> 2. **~10-minute live check of ADR-090** — this session's entire lesson is that three separate capability gaps were invisible to tests and only surfaced by RUNNING the app. Launch via `scripts/run-desktop-dev.ps1 -NoReranker` (⚠️ read the two corrections below on why the DEFAULT invocation cannot verify this) and confirm: (a) the window opens **immediately**, not after ~40s; (b) with the model already present, the log shows `ranking model warm-up requested at startup (ADR-090)` and the first search is FAST (no ~24s stall); (c) typing a word fires exactly **one** `reranked search complete` on Enter, not one per keystroke-pause; (d) the "getting your recall engine ready" line appears only while genuinely preparing and **clears itself** — a line that never clears is the specific failure `engine_ready_state_matches_between_rust_and_the_frontend` guards against, and it would be a permanent lie rather than a temporary silence.
> 3. **Then MSI packaging** — blockers (a)-(f) below, unchanged.
>
