...
## 1 · 🟢 NEXT SESSION OPENER — 🔗 GRAPH FEATURE COMPLETE (ADR-SEC-002 Part 2): self-cleaning + measured + WIRED INTO THE READ PATH. Two commits this session (stale-links `8339b04` CI-green; read-path wiring local-DoD-green, READY TO COMMIT)

> ### ▶️ START HERE — current state (2026-06-27, session 15 close)
> **The graph feature is FINISHED.** It is now (1) self-cleaning — stale edges retire when a fact stops being current; (2) measured — real-Phi-4 extraction clears the quality bar; and (3) **wired into the read/answer path** as an additive recall channel for relational queries. Two commits this session:
>   - **Commit 1 — stale-link rewrite (`8339b04`): COMMITTED + PUSHED + CI GREEN** (run `28292683623`, `success` across `[ubuntu, windows, macos]`). `source_memory_id` provenance + retire-on-change. Detail + ADR in the session-15 block below.
>   - **Commit 2 — measure + wire the graph into the read path: local DoD GREEN, READY TO COMMIT (not yet committed).** See the session-15-continued block below.
>
> **The measurement (the gate):** a 20-fact hand-labelled corpus scored the real production extraction path. After a light prompt harden, **relationship endpoint-precision 0.92, recall 0.88, entity-precision 0.89** — clears the wiring bar (the decisive metric, endpoint-P, governs whether the graph connects the *right* things; the entity-type near-miss is benign type-ambiguity that doesn't affect name-based traversal). Zero fabricated/wrong edges in the diffs.
>
> **The wiring (`GraphRetriever`):** deterministic, NO LLM (architecture lock honoured). A relational query → match query terms to known entity names (the "the user" hub excluded) → pull the live edges touching them (bidirectional) → follow each edge's `source_memory_id` to its memory → union those into the reranker pool. **Additive + recall-safe:** the reranker re-scores the whole pool, so a graph hit can only be *offered*; it never displaces a semantic result or lowers precision. Behind an on/off switch (`RerankedRetriever::with_graph(None)` = today's exact behaviour, instant rollback).
>
> **DO FIRST next session:** commit + push Commit 2, then verify ITS CI green across `[ubuntu, windows, macos]` (ubuntu disk flake → `gh run rerun --failed`).
>
> **THEN — pick the next arc (founder's choice):**
>   1. **Live dogfood the graph read-path** — seed a relational vault (sister→hospital, etc.), run real `memory_search`, confirm the graph surfaces the connected fact a pure-semantic query would miss. Proves the feature end-to-end on real data.
>   2. **A1 + ADR-083 contradiction live dogfood** (closes Findings B/E) · **10k full-sweep scale run** (opportunistic).
>   - **Deferred:** read-precision gaps (LEFT TO THE AGENT) · cross-device sync (until paying users) · graph extraction entity-type precision (benign, doesn't affect read path — don't chase it).
>
> **Already SHIPPED — the engine (all green + committed except Commit 2):** local encrypted storage (SQLite/SQLCipher + sealed LanceDB + sealed graph) · write+retrieval pipeline (BGE embed + Qwen3 reranker **+ now a knowledge-graph recall channel**) · boundaries (mandatory access control) · sleep consolidator (merge / contradiction / decay / scheduling / checkpoint+rollback / REPORT, incremental) · A1 cold archive · **knowledge graph: extracted + self-cleaning + measured + read at answer time** · **multi-agent daemon (ADR-SEC-001) — built + live-proven** · MCP stdio + daemon HTTP, cross-agent proven (Claude / Cursor / Codex).
>
...
> ### 🆕 2026-06-27 (session 15, continued) — GRAPH MEASURED + WIRED INTO THE READ PATH (ADR-SEC-002 Part 2, step 2). Local DoD GREEN; READY TO COMMIT (Commit 2).
...
> **▶️ NEXT:** commit + push Commit 2 + verify CI; then live-dogfood the graph read-path (see §1).
>
