...
**Last updated:** 2026-06-05 (**SCALE TEST + read recall-union fix (ADR-069) — DoD-GREEN, COMMITTING THIS SESSION.** Built the 100-fact scale correctness harness (`scale_eval.rs` + `scale_eval.json`); first scorecard exposed a real bug: the subject-less cello fact (`"Plays the cello…"`) vanished from read AND search at 100 facts. Root cause (measured, not guessed): NOT BGE quality (pure-BGE ranked it #6/100) — the **hybrid's RRF fusion starves keyword-less pure-semantic matches** (every "The user …" fact gets a 2nd RRF term from the incidental "user" token; the keyword-less cello drops below the top-20 cap, so the reranker never sees it). **Fix = ADR-069 read recall-union:** `memory_read` now unions the semantic channel's top-N onto the hybrid hits before reranking (`RERANK_CANDIDATE_CAP` → 2×). Re-run scorecard: **read recall 9/10→10/10, cello at rank 1, false-abstain still 0, no collateral.** Validated LIVE in Claude Desktop (cello surfaces; cat→"no cat", salary→"no salary" both correctly declined by the agent → **Decision A settled: recall-first "false-answers" are harmless at the user layer; leave the −2.5 floor alone**). **All five DoD gates GREEN this session (2026-06-05): fmt ✅ · clippy --workspace --all-targets -D warnings ✅ 0 · build --workspace ✅ · test -p vault-retrieval ✅ 63 · test -p vault-app ✅ 58.** (One clippy `doc-lazy-continuation` nit in the ADR-069 doc-comment fixed; no logic change.) Cross-agent connection to **Kimi CLI proven** (`kimi mcp test` → 5 tools) but interactive Kimi blocked by a server-startup/multi-client infra mismatch (see opener). **NEXT (after this commit + CI green): lazy-load the reranker for MCP-client compat; then re-run Kimi independent validation.** Prior openers HISTORICAL.)

---

## 🟦 NEXT SESSION OPENER (2026-06-05) — **SCALE TEST + read recall-union fix (ADR-069). DoD-GREEN, committed this session. Next: verify CI green → lazy-reranker → Kimi re-test.** — READ THIS FIRST
