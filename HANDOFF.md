...
**Active task:** **T0.2.0 Phase 0b — vault-storage API-drift sweep on lancedb 0.27.** Phase 0a-fix landed ADR-038 (concurrent-upsert serialisation + LANCE_MEM_POOL_SIZE shell-level ceiling); 216/216 vault-storage tests pass (382/382 workspace-wide, 17 pre-existing ignored). Phase 0b is a verification step before Phase 0c re-spikes at-rest encryption against the upgraded stack.
...
| **0b** | NOT YET STARTED | Verification sweep: vault-storage API drift surfaced by 0a/0a-fix. Library code compiles clean against lancedb 0.27 and 216/216 vault-storage tests pass (382/382 workspace-wide, 17 pre-existing ignored) post-0a-fix. Phase 0b will be a brief audit step before Phase 0c — kicks off after this commit + CI green. |
