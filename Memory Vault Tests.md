...
**★ HEADLINE THIS RUN — RECALL-FIRST READ (ADR-066, 2026-06-02). This is the make-or-break sprint.** The read no longer tries to be a strict relevance *authority* via the cross-encoder. The reranker now (1) re-orders candidates and (2) drops only deep *no-signal* junk (a low floor ≈ −2.5), and the calling agent makes the fine relevance call. This fixes the **synonym-gap over-abstention** that triggered the scrap debate: a query and a stored fact using *different words for the same thing* (e.g. "what **food** do I like?" ↔ "favourite **cuisine** is Japanese") now surfaces instead of abstaining. **The live ship-gate for this run is the synonym-gap reads — above all B3's `"what food does the user like?"` → must surface PRECFOOD** (and it carries the `PRECFOOD` marker that previously forced the abstain, so this is the exact failing case). The A6 no-signal abstention (`"blood type"`) must STILL hold — recall-first must not become a firehose. Already proven green on the real BGE + reranker in `read_no_keyword_overlap::synonym_gap_reads_surface_recall_first_and_still_abstain_on_no_signal`; this dogfood confirms it in Claude Desktop. **C4 is executed by Claude Code (automated probe) — skip it in Desktop.**

**Carried-over headline — A5 knowledge-update contradiction fires via nearest-neighbor candidates (ADR-065, 2026-06-01):** the consolidator no longer relies on K-means topic grouping (which split the conflicting pair into different buckets so the judge never saw it and A5 silently failed). It now pairs each fact with its nearest cosine neighbors above a **0.70** similarity floor and judges those pairs with the existing Phi-4 + recency logic. **B1 (Tesla→Rivian) is the live ship-gate for this fix.** Expected: the run report shows the contradiction detected and ONLY the older Tesla invalidated. The Tesla/Rivian pair (measured cosine 0.823, mutual #1 neighbors) is the *only* candidate the floor admits in this seed set — every other seeded fact sits ≤0.634, below the floor — so Claude Code confirms a small `candidate_pairs` count + a single invalidation in the log.
...
- **C4 — content ceiling — ⚙️ CLAUDE CODE EXECUTES THIS (Desktop: SKIP).** Desktop cannot emit a literal 50K+ payload, so Claude Code runs it as an automated probe (`vault-app/tests/content_ceiling.rs`) through the real `Adapter::write` path against an *isolated* temp vault — measuring the ceiling without polluting `testeval`. Result recorded in the run notes; do NOT seed CAP_OK probes in the dogfood vault. (Original Desktop instructions retained below for reference only.) This probes the SERVER's storage ceiling, so the **full literal payload MUST reach the server** — a shortened payload silently invalidates the test. `memory_write` three facts to `testeval`, each of the form `CAP_OK_<n>_START` + filler + `CAP_OK_<n>_END`, where the **filler is the 10-character block `ABCDEFGHIJ` repeated EXACTLY**:
